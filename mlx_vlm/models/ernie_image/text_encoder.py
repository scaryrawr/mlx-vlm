...
class _DecoderLayer(nn.Module):
    ...
    def __call__(
        ...
        hidden = x + self.self_attn(self.input_layernorm(x), mask, cos, sin)
...
class _TextModel(nn.Module):
    def __init__(
        ...
        *,
        ...
        self._inv_freq = 1.0 / (
            rope_theta ** (mx.arange(0, head_dim, 2, dtype=mx.float32) / head_dim)
        )

    def rotary(self, seq_len: int, dtype: mx.Dtype) -> tuple[mx.array, mx.array]:
        """Build causal text rotary embeddings for a sequence length."""
        ...
        return (
            mx.cos(embedding)[None, None, :, :].astype(dtype),
            mx.sin(embedding)[None, None, :, :].astype(dtype),
        )
...
class ErnieMistralTextEncoder(nn.Module):
    """Mistral-compatible encoder returning ERNIE's penultimate hidden state."""
    ...
    def __call__(
        ...
        causal_mask = mx.where(
            ...
            mx.full((length, length), -float("inf"), dtype=mask_dtype),
            mx.zeros((length, length), dtype=mask_dtype),
        )[None, None, :, :]
        cos, sin = model.rotary(length, mask_dtype)
        ...
        for index, layer in enumerate(model.layers):
            ...
            if index == self._num_hidden_layers - 2:
                penultimate = hidden_states
        return penultimate


__all__ = ["ErnieMistralTextEncoder"]
