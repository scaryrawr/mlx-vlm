...
Preload a default with `--embedding-model <repo-or-path>`. Supported architectures: BERT, NomicBERT, XLM-RoBERTa, ModernBERT, Qwen2 (including nomic-embed-code), Qwen3-Embedding, EmbeddingGemma (gemma3), LFM2, SigLIP (text), Qwen3-VL-Embedding, and Llama-Nemotron-VL, plus LLM2Vec bidirectional Llama. ColBERT-style multi-vector models (ColIdefics3, ColQwen2.5) are also available for late-interaction use.
